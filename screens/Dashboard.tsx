...
import useGetUsers from "../api/firebase/hooks/useGetUsers";

export default function Dashboard({
  navigation,
}: RootTabScreenProps<"Dashboard">) {
  const [trigger, { data, isLoading }] = useGetUsers();

  if (isLoading) {
    return <Text>Data still loading</Text>;
  }
        ...
        <Flex style={styles.listItemBody}>
          ...
          <View>
            <Text style={styles.name}>
              {item.about.name}{" "}
              <Text style={styles.age}>({item.about.age})</Text>
            </Text>
            <Text>{item.about.bio}</Text>
          </View>
        </Flex>
      </Pressable>
    );
